...
<form action="{{ route('admin_subscription.assign') }}" method="post">
    @csrf
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div class="">
            ...
            <select id="contactPerson" name="contact_person" class="form-select" >
                @foreach($companies as $company)
    ...
    <div class=" mt-2">
        ...
        <select id="subscriptionPlan" name="subscription_id" class="form-select">
            @foreach($activeSubscriptionPlans as $plan)
                <option value="{{ $plan->plan_type }}{{$plan->id}}" data-type="{{ $plan->plan_type }}" >{{ $plan->plan_name }}</option>
            @endforeach
        </select>
    </div>
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
    <div class=" mt-2">
        ...
        <input type="date" id="startDate" name="start_date" class="form-input" onchange="formatDate()">

    </div>

    <div class=" mt-2">
        <label for="endDate">End Date</label>
        <input type="date" id="endDate" class="form-input" readonly>
        
        <input type="hidden" name="end_date" id="hiddenEndDate">
    </div>
    </div>
    <button type="submit" class="btn btn-primary mt-2">Assign Subscription Plan</button>
</form>
